...
var effects = 	[	{	"name":"SmokeExplosion",
					...
					{	"name":"FootDust",
						"animation":"landing",
						"packed":preload("res://scenes/FootDust.tscn")
					}
				]

func create_effect(effect:String,position:Vector2,scale=Vector2(1,1),fliph:bool=false):
	for obj in effects:
		if obj["name"] == effect:
			var new_effect = obj["packed"].instance()
			new_effect.set_deferred("scale",scale)
			new_effect.set_deferred("fliph",fliph)
